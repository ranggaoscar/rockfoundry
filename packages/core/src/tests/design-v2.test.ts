...
describe("Design Engine V2", () => {
  ...
  it("rejects an ugly but safety-valid prototype", () => {
    ...
    const screens = deriveScreenMap(state);
    const links = screens
      .map((screen) => `<a href="${screen.route}">${screen.name}</a>`)
      .join("");
    const files = [
      {
        path: "index.html",
        content: `<main><nav>${links}</nav><div class="blob"></div><h1>THE FUTURE OF EVERYTHING</h1></main>`,
      },
      {
        path: "styles.css",
        content:
          ".blob{width:500px;height:500px;border-radius:50%;background:#ddd;} @media(max-width:600px){}",
      },
      { path: "app.js", content: "" },
    ];
  ...
  it("parses legacy DesignSpec V1 and additive V2 fields together", () => {
    const parsed = DesignSpecSchema.parse({
      ...
      controls: "buttons",
      responsive: "stack",
      componentsV2: [{ name: "Table", purpose: "show records" }],
    });
  ...
  it("keeps old prototype artifacts readable", () => {
    const artifact = PrototypeGenerationOutputSchema.parse({
      files: [
  ...
  it("keeps legacy handoff/export structure valid", async () => {
    ...
    expect(result.documents).toMatchObject({
      BRD: expect.any(String),
      PRD: expect.any(String),
      ERD: expect.any(String),
      AGENT_HANDOFF: expect.any(String),
    });
    expect(result.metadata.fileCount).toBe(17);
    expect(result.documents.AGENT_HANDOFF).toContain("DO_NOT_INVENT.md");
    expect(result.documents.AGENT_HANDOFF).toContain(
      "The approved prototype, when included",
    );
  });
  ...
  it("preserves additive V2 metadata while defaulting omitted nested values", () => {
    const parsed = DesignSpecSchema.parse({
      ...
      responsive: "stacked",
      componentsV2: [
        { name: "Button", purpose: "Primary action", stateNotes: "" },
      ],
      screensV2: [{ screenId: "overview", primaryAction: "Create" }],
    });
  ...
  it("accepts additive quality metadata and defaults absent metadata safely", () => {
    ...
    const withMetadata = ProjectStateSchema.parse({
      ...base,
      generationMetadata: { quality: { score: 92, verdict: "PASS" } },
    });
    const withoutMetadata = ProjectStateSchema.parse({ ...base });
    expect(withMetadata.generationMetadata.quality).toEqual({
      score: 92,
      verdict: "PASS",
    });
