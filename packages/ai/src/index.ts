...
import { ApiError, MalformedJsonResponseError } from "./gateway";
import {
  AiGatewayProvider,
  InferenceRequest,
  InferenceResponse,
...
function parsePrototypeJsonValue(value: unknown) {
...
function unwrapPrototypeResponse(value: unknown, depth = 0): Record<string, unknown> {
  const parsed = parsePrototypeJsonValue(value);
  if (!isPrototypeRecord(parsed) || depth > 4) return {};
  if (
    "files" in parsed ||
    "artifacts" in parsed ||
...
function prototypeFileContent(value: unknown) {
  if (typeof value === "string") return value;
  if (!isPrototypeRecord(value)) return undefined;
...
function prototypeFilesFrom(value: unknown) {
  ...
  const collect = (path: unknown, content: unknown) => {
    if (
      typeof path !== "string" ||
      !PROTOTYPE_PATHS.includes(path as (typeof PROTOTYPE_PATHS)[number])
    )
      return;
    const resolved = prototypeFileContent(content);
  ...
  if (Array.isArray(listed)) {
    for (const entry of listed) {
      if (isPrototypeRecord(entry))
        collect(entry.path ?? entry.name ?? entry.fileName, entry);
    }
  } else if (isPrototypeRecord(listed)) {
...
function prototypeMetadataFrom(value: unknown) {
  const outer = parsePrototypeJsonValue(value);
  const responses = [
    unwrapPrototypeResponse(value),
    ...(isPrototypeRecord(outer) ? [outer] : []),
  ];
...
function normalizePrototypeOutput(value: unknown, preserve?: unknown) {
  const files = { ...prototypeFilesFrom(preserve), ...prototypeFilesFrom(value) };
